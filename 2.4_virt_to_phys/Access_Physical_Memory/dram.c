...
loff_t my_llseek( struct file *file, loff_t offset, int whence )
{
	...
	switch( whence )
		{
		case 0: newpos = offset; break;			// SEEK_SET, set the abs value
		case 1: newpos = file->f_pos + offset; break; 	// SEEK_CUR, set from the last value
		case 2: newpos = dram_size + offset; break; 	// SEEK_END, count from the end
		}
